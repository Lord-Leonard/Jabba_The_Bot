...
func (s *Server) handleCommands(w http.ResponseWriter, r *http.Request) {
	...
	if s.commands == nil {
		writeJSON(w, http.StatusServiceUnavailable, commandResponse{
			Accepted: false,
			Error: &commandError{
				Code:    "COMMANDS_UNAVAILABLE",
				Message: "command handler not configured",
			},
		})
		return
	}
...
func (s *Server) executeCommand(req commandRequest) error {
	...
	case "enqueue":
		...
		_, err := s.commands.Enqueue(query, ports.RequestSource{Platform: "web"})
		if err != nil {
			return err
		}
		return nil
	case "pause":
		err := s.commands.Pause()
		if err != nil {
			return err
		}
		return nil
	case "resume":
		err := s.commands.Resume()
		if err != nil {
			return err
		}
		return nil
	...
	case "seek":
		...
		err := s.commands.Seek(time.Duration(positionSec * float64(time.Second)))
		if err != nil {
			return err
		}
		return nil
	case "stop":
		clearQueue := true
		if v, ok := req.Payload["clearQueue"].(bool); ok {
			clearQueue = v
		}
		err := s.commands.Stop(clearQueue)
		if err != nil {
			return err
		}
		return nil
