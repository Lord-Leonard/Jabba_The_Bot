...
//	Consume(source playback.AudioStream) error
//}
//
...
//	m.mu.Lock()
//	ctrl := m.current
//	var stream playback.AudioStream
//	if ctrl != nil {
//		stream = ctrl.stream
//	}
//	m.mu.Unlock()
...
//	}
//
//	seekable, ok := stream.(playback.SeekableAudioStream)
//	if !ok {
...
//}
//
//func (m *Manager) maybeTriggerAutoplay(track Track) {
...
//
//		added := 0
//		prefetchCandidates := make([]Track, 0, m.autoplayPrefetch)
//		for i := range recs {
...
//}
//
//func (m *Manager) enqueueAutoTrack(track Track) bool {
...
//}
//
//func (m *Manager) prefetchTracks(tracks []Track) {
//	go func(items []Track) {
...
//}
//
//func (m *Manager) prefetchNearEnd(ctrl *playbackControl, track Track, stream playback.AudioStream) {
//	if m.autoplayPrefetch <= 0 {
//		return
//	}
//
//	target := m.prefetchFallback
//	if timed, ok := stream.(playback.TimedAudioStream); ok {
...
//	}
//
//	if _, ok := stream.(playback.TimedAudioStream); !ok {
...
//			return
//		case <-ticker.C:
//			elapsed := stream.(playback.TimedAudioStream).Position()
//			if elapsed >= target {
...
//}
//
//func (m *Manager) collectPrefetchTargets(max int) []Track {
//	if max <= 0 {
//		return nil
//	}
//
...
//}
//
//func (m *Manager) resolveQueueTrack(ctx context.Context, idx int) (Track, bool) {
//	m.mu.Lock()
...
//}
//
//func (m *Manager) setCurrentStream(ctrl *playbackControl, stream playback.AudioStream) {
//	m.mu.Lock()
//	defer m.mu.Unlock()
//	if m.current == ctrl {
...
//	if m.current != nil {
//		if timed, ok := m.current.stream.(playback.TimedAudioStream); ok {
//			pos = timed.Position()
//			dur = timed.Duration()
//		}
//	}
...
//}
//
//func (m *Manager) setNowPlaying(track Track) {
//	m.mu.Lock()
//	trackCopy := track
...
//}
//
//func (m *Manager) emitNowPlaying(track Track) {
